...
_lr_productions = [
  ...
  ('program -> VAR declarations BEGIN commands END','program',5,'p_program_var','parser.py',15),
  ('program -> BEGIN commands END','program',3,'p_program','parser.py',22),
  ('declarations -> declarations COMMA PIDENTIFIER','declarations',3,'p_declarations_next','parser.py',36),
  ('declarations -> declarations COMMA PIDENTIFIER LQBRACKET NUM COLON NUM RQBRACKET','declarations',8,'p_declarations_array_next','parser.py',43),
  ('declarations -> PIDENTIFIER','declarations',1,'p_declarations','parser.py',49),
  ('declarations -> PIDENTIFIER LQBRACKET NUM COLON NUM RQBRACKET','declarations',6,'p_declarations_array','parser.py',55),
  ('commands -> command','commands',1,'p_commands_start','parser.py',69),
  ('commands -> commands command','commands',2,'p_commands_next','parser.py',73),
  ('command -> identifier ASSIGN expression SEMICOLON','command',4,'p_command_assign','parser.py',96),
  ('command -> IF condition THEN commands ELSE commands ENDIF','command',7,'p_command_ifthenelse','parser.py',98),
  ('command -> IF condition THEN commands ENDIF','command',5,'p_command_ifthen','parser.py',100),
  ('command -> WHILE condition DO commands ENDWHILE','command',5,'p_command_whiledo','parser.py',102),
  ('command -> REPEAT commands UNTIL condition SEMICOLON','command',5,'p_command_repeatuntil','parser.py',104),
  ('command -> FOR PIDENTIFIER FROM value TO value DO commands ENDFOR','command',9,'p_command_forfromtodo','parser.py',106),
  ('command -> FOR PIDENTIFIER FROM value DOWNTO value DO commands ENDFOR','command',9,'p_command_forfromdowntodo','parser.py',108),
  ('command -> READ identifier SEMICOLON','command',3,'p_command_read','parser.py',110),
  ('command -> WRITE value SEMICOLON','command',3,'p_command_write','parser.py',112),
  ('expression -> value','expression',1,'p_expression','parser.py',117),
  ('expression -> value PLUS value','expression',3,'p_expression','parser.py',118),
  ('expression -> value MINUS value','expression',3,'p_expression','parser.py',119),
  ('expression -> value TIMES value','expression',3,'p_expression','parser.py',120),
  ('expression -> value DIV value','expression',3,'p_expression','parser.py',121),
  ('expression -> value MOD value','expression',3,'p_expression','parser.py',122),
  ('condition -> value EQ value','condition',3,'p_condition','parser.py',127),
  ('condition -> value NEQ value','condition',3,'p_condition','parser.py',128),
  ('condition -> value LE value','condition',3,'p_condition','parser.py',129),
  ('condition -> value GE value','condition',3,'p_condition','parser.py',130),
  ('condition -> value LEQ value','condition',3,'p_condition','parser.py',131),
  ('condition -> value GEQ value','condition',3,'p_condition','parser.py',132),
  ('value -> NUM','value',1,'p_value','parser.py',137),
  ('value -> identifier','value',1,'p_value','parser.py',138),
  ('identifier -> PIDENTIFIER','identifier',1,'p_identifier','parser.py',142),
  ('identifier -> PIDENTIFIER LQBRACKET PIDENTIFIER RQBRACKET','identifier',4,'p_identifier','parser.py',143),
  ('identifier -> PIDENTIFIER LQBRACKET NUM RQBRACKET','identifier',4,'p_identifier','parser.py',144),
]
