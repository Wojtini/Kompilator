...
from misc.declarations import DeclarationArray
...
def p_declarations_next(p):
    'declarations : declarations COMMA PIDENTIFIER'
    var = DeclarationVar(p[3],False,False,p.lineno(3))
    p[1].append(var)
    p[0] = p[1]
...
def p_declarations_array_next(p):
    ...
    var = DeclarationArray(p[3],int(p[5]),int(p[7]),p.lineno(3))
    p[1].append(var)
    p[0] = p[1]

def p_declarations(p):
    'declarations : PIDENTIFIER'
    var = DeclarationVar(p[1],False,False,p.lineno(1))
    p[0] = []
    p[0].append(var)

def p_declarations_array(p):
    'declarations : PIDENTIFIER LQBRACKET NUM COLON NUM RQBRACKET'
    var = DeclarationArray(p[1],p[3],p[5],p.lineno(1))
    p[0] = []
    p[0].append(var)
...
def p_command_assign(p):
    'command : identifier ASSIGN expression SEMICOLON'
    p[0] = commands.CommandAssign(p.lineno(2))
...
def p_command_read(p):
    ...
    p[0] = commands.CommandRead(p.lineno(1))
def p_command_write(p):
    'command : WRITE value SEMICOLON'
    p[0] = commands.CommandWrite(p.lineno(1))
